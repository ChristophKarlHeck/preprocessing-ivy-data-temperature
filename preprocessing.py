...
DATA_DIR = "/home/chris/experiment_data/09.01.25-15.01.25" # containing P3, P5 and P6 (temperature data)
...
df_p5_resampled["CH1_smooth"] = df_p5_resampled["CH1_mv"].rolling(window=WINDOW_SIZE).mean().dropna()
df_p5_resampled["CH2_smooth"] = df_p5_resampled["CH2_mv"].rolling(window=WINDOW_SIZE).mean().dropna()
...
#P5 CH1
axs[2].plot(df_p5_resampled.index, df_p5_resampled["CH1_mv"], label="Resampled CH1")
axs[2].plot(df_p5_resampled.index, df_p5_resampled["CH1_smooth"], label="Smoothed CH1", linestyle="--")
axs[2].set_title("P5 CH1 Resampled, Smoothed, and Scaled")
axs[2].set_ylabel("mV")
axs[2].legend()
axs[2].grid()

# P5 CH2
axs[3].plot(df_p5_resampled.index, df_p5_resampled["CH2_mv"], label="Resampled CH2")
axs[3].plot(df_p5_resampled.index, df_p5_resampled["CH2_smooth"], label="Smoothed CH2", linestyle="--")
axs[3].set_title("P5 CH2 Resampled, Smoothed, and Scaled")
axs[3].set_ylabel("mV")
axs[3].legend()
axs[3].grid()

axs[4].plot(df_p5_resampled.index, df_p5_resampled["CH1_smooth_scaled"], label="Scaled CH1", linestyle=":")
axs[4].plot(df_p5_resampled.index, df_p5_resampled["CH2_smooth_scaled"], label="Scaled CH2", linestyle="--")
axs[4].plot(df_p3_resampled.index, df_p3_resampled["CH1_smooth_scaled"], label="Scaled CH1", linestyle=":")
axs[4].plot(df_p3_resampled.index, df_p3_resampled["CH2_smooth_scaled"], label="Scaled CH2", linestyle="--")
axs[4].set_title("CH1, CH2, smoothed and scaled")
...
# axs[4].plot(df_temp_resampled.index, df_temp_resampled['avg_leaf_temp'], label='Average Leaf Temperature', alpha=0.7)
# axs[4].plot(df_temp_resampled.index, df_temp_resampled['avg_air_temp'], label='Average Air Temperature', alpha=0.7)
# axs[4].set_title("Temperature Data (1 Hz)")
# axs[4].set_xlabel("Datetime")
# axs[4].set_ylabel("°C")
# axs[4].legend()
# axs[4].grid()
...
df_p5_resampled.to_csv(os.path.join(preprocessed_dir, "p5_preprocessed.csv"))
